...
namespace node_wrapper
{
    ...
    Local<Array> array_intermediate(Isolate *isolate, vector<vector<int>> board){
        Local<Array> final_array = Array::New(isolate, board.size());
        Local<Context> context = isolate->GetCurrentContext();

        for (int i = 0; i < board.size(); i++){
            vector<int> row = board[i];
            Local<Array> row_array = Array::New(isolate, row.size());

            for (int ii = 0; ii < row.size(); ii++){
                row_array->Set(context, ii, v8::Integer::New(isolate, row[ii]));
            }

            final_array->Set(context, i, row_array);
        }
